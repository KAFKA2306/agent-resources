...
def _text_is_informative(name, value, *, minimum_length=16):
    ...
    if any(phrase in lower for phrase in _REJECT_PURPOSE_PHRASES):
        return False
...
def _readme_candidates(readme_text):
    ...
    def append_candidate(kind, value, *, level=None):
    ...
    for index, raw_line in enumerate(lines):
        ...
        if heading_match:
            ...
            heading = _clean_markdown_text(heading_match.group(2))
            if level == 1:
                append_candidate("heading", heading, level=1)
                section = None
            else:
                section = heading
            continue
...
def _purpose_section(section):
    if section is None:
        return True
    normalized = _clean_markdown_text(section).casefold()
    return any(marker in normalized for marker in _PURPOSE_SECTION_MARKERS)
...
def _candidate_score(name, item):
    candidate = item["text"]
    kind = item["kind"]
    section = item.get("section")
    lower = candidate.casefold()

    if kind == "paragraph" and not _purpose_section(section):
        return -1
    if kind == "heading" and item.get("level") != 1:
        return -1
    ...
    if candidate.count("http://") + candidate.count("https://") >= 1 and len(candidate) < 120:
        return -1
    if len(re.findall(r"(?:^|\s)[\w.-]+/[\w./-]+", candidate)) >= 2:
        return -1
    if re.search(r"\b(?:cuda|cpython|python|pytorch|node|npm|pip)\s*[<>=]*\s*\d", lower):
        return -1

    purpose_hit = any(marker in lower for marker in _PURPOSE_MARKERS)
    action_hits = sum(1 for marker in _ACTION_MARKERS if marker in lower)
    if not purpose_hit and action_hits == 0:
        return -1
    if kind == "heading" and not purpose_hit:
        return -1
...
def _readme_semantic(facts):
    name = facts["name"]
    scored = []
    for order, item in enumerate(_readme_candidates(facts.get("readmeText"))[:40]):
        candidate = item["text"]
        lower = candidate.casefold()
        if lower.startswith(
            (
            ...
            continue
        score = _candidate_score(name, item)
        if score >= 5:
