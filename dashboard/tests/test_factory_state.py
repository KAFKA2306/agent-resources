...
class FactoryStateTests(unittest.TestCase):
    def test_verified_requires_merge_deploy_and_probe(self):
        ...
        self.assertEqual(
            derive_workline_state({**base, "probed": True})["state"], "VERIFIED"
        )

    def test_failure_is_not_pass(self):
        item = derive_workline_state(
            {
                "id": 390,
                "stage": "write_back",
                "conclusion": "failure",
                "failure_class": "permission_mismatch",
            }
        )
    ...
    def test_generator_evidence_is_materialized_once(self):
        signals = (
            signal
            for signal in [
                {
        ...
        state = build_factory_state(
            signals, main_sha="abc", generated_at="2026-09-22T01:00:00Z"
        )

        self.assertEqual(state["metrics"]["firstPassYield"], 1.0)
        self.assertEqual(state["metrics"]["selfHealRate"], 1.0)
        self.assertEqual(state["failurePareto"], [{"failureClass": "flaky_test", "count": 1}])

    def test_human_authority_failure_is_not_counted_as_healable(self):
        state = build_factory_state(
            [
                {"id": 1, "failure_class": "initial_external_auth"},
                {
                    "id": 2,
                    "conclusion": "failure",
                    "failure_class": "permission_mismatch",
